...
public class LatePaymentTest {
  @Test void score_drops_with_late_payments(){
    User u=new User("Luisa","luisa@example.com",3000000);
    CreditCardAccount cc=new CreditCardAccount("1","Visa",1000000,0.3, LocalDate.now().minusMonths(12), 2000000);
    cc.addPayment(new PaymentRecord(LocalDate.now().minusMonths(1), 200000, false)); u.addAccount(cc);
    ScoreCalculator calc=new WeightedScoreCalculator(Map.of("utilization",0.4,"punctuality",0.3,"age",0.2,"mix",0.1));
    int score=calc.compute(u).getScore(); assertTrue(score<800);
  }
}
